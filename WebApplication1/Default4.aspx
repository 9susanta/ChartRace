...
<style>
  ...
  <script>
      ...
      d3.csv('output.csv').then(function (data) {
          debugger;
          data.forEach((d, i) => d.color = d3.hsl(Math.random() * 360, 0.75, 0.75));

          let yearSlice = data.sort((a, b) => {
              return d3.descending(+a[year], +b[year])
          }).slice(0, top_n);

          yearSlice.forEach((d, i) => d.rank = i);

          console.log('yearSlice2: ', yearSlice)

          let x = d3.scaleLinear()
              .domain([0, d3.max(yearSlice, d => +d[year])])
          ...
          let z = d3.scaleBand()
              .domain(yearSlice.map(function (d) { return d.name; }))
              .range([55, height - 25])
          ...
          let ticker = d3.interval(e => {
              ...
              x.domain([0, d3.max(yearSlice, d => +d[year])]);

              z.domain(yearSlice.map(function (d) { return d.name; }));
              ...
              if (year == "2019.11") ticker.stop();

              if (!year.toString().includes(".12")) {
